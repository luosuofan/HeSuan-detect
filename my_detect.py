...
def guide(person, max_num):
    # host = Linux('192.168.149.1', 'ubuntu', 'hiwonder')
    # host.connect()
    pt = pyttsx3.init()
    pt.say("核酸采样系统已开启。图像显示界面处理中，请稍后...")
    pt.runAndWait()
    while person.uistate is False:
        time.sleep(1)
    # time.sleep(15)
    ...
    pt.runAndWait()
    pt.say("第二步，将咽拭子伸到咽喉方框采样区，在两侧扁桃体擦拭三到四次，中间擦拭三到四次")
    pt.runAndWait()
    print('请按示范操作，将咽拭子伸到方框采样区，在两侧擦拭三到四次，中间扁桃体擦拭三到四次')
    max_num_sampling = max_num  # 采样到N次采样成功的帧，判定为采样成功
    timeout_1 = 60  # 采样时超时限制
    my_time = 0
    while person.sampling_count < max_num_sampling and my_time < timeout_1:
        time.sleep(1)
        my_time += 1
    ...
    my_time = 0
    timeout_2 = 60  # 规定时间内完成拭子放入(s)
    lock.acquire()
    while not (person.flag_1 and person.flag_2 and person.flag_3) and my_time < timeout_2:
        time.sleep(1)
        my_time += 1
    lock.release()
    if my_time == timeout_2:
        pt.say("您已超时，核酸检测失败")
        pt.runAndWait()
        print('您已超时，核酸检测失败')
    else:
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            for re in detections:
                if re['class'] == "throat" and re['conf'] >= 0.45:
                    a = int(max(re['position'][2], re['position'][3]) / 2)
                    b = int(min(re['position'][2], re['position'][3]) / 2)
                    # cv2.circle(im0, re['center_xy'], 5, (255, 255, 255), cv2.FILLED)
                    cv2.ellipse(im0, re['center_xy'], (a, b), 0, 0, 360, (255, 255, 255), 1)
                if re['class'] == "sampling" and re['conf'] >= 0.6:
                    if p1.sampling_count < MAX_NUM:
                        p1.sampling_count += 1  #
                        print("采样次数：", + p1.sampling_count)
                    # cv2.circle(im0, re['center_xy'], 5, (0, 0, 255), cv2.FILLED)
                elif re['class'] == "sampling2" and re['conf'] >= 0.6:
                    if p1.sampling_count < MAX_NUM:
                        p1.sampling_count += 0.5  #
                        print("采样次数：", + int(p1.sampling_count))
                else:
                    if re['class'] == "swab" and re['conf'] >= 0.8:
                        p1.flag_1 = True
                    if re['class'] == "Intube" and re['conf'] >= 0.75 and p1.flag_1:
                        p1.flag_2 = True
                    if re['class'] == "stick" and re['conf'] >= 0.65 and p1.flag_1 and p1.flag_2:
                        p1.flag_3 = True
...
if __name__ == "__main__":
    ...
    opt = parse_opt()
    th_list = []
    thread_1 = threading.Thread(target=guide, args=[p1, MAX_NUM])
